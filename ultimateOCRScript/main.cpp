...
std::string lang = "dig+itLetters+symbols+digits2+letters2"; // file on disk is "my_trained_file.traineddata"
...
string extractContours(Mat& image,vector< vector<Point> > contours_poly){
	...
	//Loop through all contours to extract
		 for( int i = 0; i< contours_poly.size(); i++ ){
            ...
            Rect prevR;
		 	if(i != 0){
		 		Rect prevR = boundingRect( Mat(contours_poly[i-1]) );
		 	}
...
void getContours(std::string filename)
{
  ...
  cv::GaussianBlur(img,img,size,0);
   adaptiveThreshold(img, img,255,CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY_INV,35,10);           // original blockSize  was 75
  //cv::bitwise_not(img, img);

    cv::imshow("Threshold effect", img);
  ...
  cv::RotatedRect box = cv::minAreaRect(cv::Mat(points));                                           // CUT OFF BLANK MARGINS OUT OF THE IMAGE
  ...
  for(int i = 0; i < 4; ++i)
   ...
   cv::Mat rotated;
   cv::warpAffine(img2, rotated, rot_mat, img.size(), cv::INTER_LINEAR, cv::BORDER_TRANSPARENT);                                             // HERE PROGRAMM CRASHES DUE TO WARPAFFINE EXCEPTION
...
cvtColor(cropped3,cropped3,CV_GRAY2RGB);
 ...
 vector<vector<Point> > contours;
  ...
  /// Find contours
  cv:: findContours( cropped, contours, hierarchy, CV_RETR_TREE, CV_CHAIN_APPROX_TC89_KCOS, Point(0, 0) );                                              // HERE PROGRAM CRASHES   :    error: (-206) Unrecognized or unsupported array type in function cvGetMat
...
//	tess_api.SetVariable("tessedit_char_whitelist", "0123456789xyz+-()/*^");
//	tess_api.SetVariable("tessedit_char_blacklist", "H");
	tess_api.Init("F:\\tesseract\\tessdata", lang.c_str(), tesseract::OEM_TESSERACT_ONLY);
